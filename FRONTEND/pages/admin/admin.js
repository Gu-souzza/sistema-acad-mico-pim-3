...
function montarProdutoFormData(id) {
    ...
    if (imagens.length) {
        formData.append("ImagemArquivo", imagens[0]);
...
function validarImagensProduto(arquivos) {
    ...
    const invalida = arquivos.find((arquivo) => !formatosPermitidos.includes(arquivo.type));
    if (invalida) {
        return "Use apenas imagens PNG, JPEG/JPG ou WEBP.";
    }
...
function obterImagemProdutoAdmin(produto) {
    ...
    const imagem = produto?.imagemUrl || produto?.ImagemUrl;
    if (!imagem) return "../../assets/logo zf.png";
    if (imagem.startsWith("http") || imagem.startsWith("../") || imagem.startsWith("./")) return imagem;
    if (imagem.startsWith("/")) return `${ZeroFrameApi.BASE_URL}${imagem}`;
    return imagem;
}
...
function renderizarProdutos(produtos) {
    ...
    produtos.forEach((produto) => {
        ...
        tr.append(
            ...
            criarTdAcoes([
                criarBotao("Editar", "btn-edit", () => abrirModalProduto(produto.id || produto.Id)),
                criarBotao("Remover", "btn-delete", () => removerProduto(produto.id || produto.Id))
            ])
        );
...
window.abrirModalProduto = function abrirModalProduto(produtoId = null) {
    ...
    if (produtoId) {
        const produto = produtosGlobais.find((item) => (item.id || item.Id) === produtoId);
        if (!produto) return;
        ...
        const imagemAtual = produto.imagemUrl || produto.ImagemUrl || "";
...
window.removerProduto = async function removerProduto(produtoId) {
    if (!produtoId || !confirm("Remover este produto?")) return;

    try {
        await ZeroFrameApi.request(`/api/produtos/${produtoId}`, { method: "DELETE" });
...
async function carregarCategorias() {
    try {
        ...
        if (!categoriasGlobais.length) {
            categoriasGlobais = carregarCategoriasLocal();
        }
...
async function salvarCategoria(event) {
    ...
    try {
        ...
        salvarCategoriaLocal(id, categoria);
...
function preencherFormularioCategoria(id) {
    const categoria = categoriasGlobais.find((item) => Number(item.id || item.Id) === id);
    if (!categoria) return;
...
function renderizarClientes(clientes) {
    ...
    clientes.forEach((cliente) => {
        ...
        tr.append(
            ...
            criarTdAcoes([criarBotao("Ver Compras", "btn-edit", () => filtrarPedidosPorCliente(id))])
        );
        tbody.appendChild(tr);
    });
}
